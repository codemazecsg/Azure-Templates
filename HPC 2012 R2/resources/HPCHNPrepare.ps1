...
try
{
    ...
    if($RemoteDB.IsPresent)
    {
        ...
        foreach($db in $DBDic.Keys)
        {
			...
			$dbNameValue = $DBDic[$db]
			Invoke-Sqlcmd -ServerInstance $DBServerInstance -Database "master" -Query "if exist (select database_id from sys.databases where name = '$dbNameValue') drop database $dbNameValue;"
			Invoke-Sqlcmd -ServerInstance $DBServerInstance -Database "master" -Query "create database [$dbNameValue]"
            Invoke-Sqlcmd -ServerInstance $DBServerInstance -Database $dbNameValue -InputFile "$env:temp\$sqlfilename" -QueryTimeout 300 -ErrorAction SilentlyContinue
            Invoke-Sqlcmd -ServerInstance $DBServerInstance -Database $dbNameValue -InputFile "$HPCBinPath\AddDbUserForHpcService.sql" -Variable "TargetAccount=$machineAccount" -QueryTimeout 300
        }
